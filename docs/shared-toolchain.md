...
Consumer repositories remain independent GitHub repositories. Their own CI workflows check out both the consumer and the shared Toolchain, then run the consumer's thin `test` / `lint` / `build` scripts against the shared dependencies.

CI reconstructs the same local directory layout inside the runner:
...
This keeps local and CI path behavior identical while keeping the reusable build configuration and dependency graph centralized in the Toolchain repository.
...
7. Consumer CI checks out `uikawinwing/tavern_helper_toolchain` beside the project, installs dependencies only in Toolchain, then runs the consumer's verification/build scripts.
8. Consumer release workflows keep their own branch-protection, commit and tag policy; Toolchain provides build behavior, not release authority.
9. Consumer repos may rebuild against the latest central Toolchain, but they do not create dependency/template branches.

## Safety boundary

Do not copy shared template files back into consumer repos to fix build errors. Fix the shared Toolchain contract instead.

`project-build.mjs` and this document are local Toolchain infrastructure and are protected from template-sync replacement via `.github/.templatesyncignore`.
...
5. Update CI to checkout `uikawinwing/tavern_helper_toolchain` beside the consumer and install dependencies in Toolchain before running the consumer's own verification/release scripts.
6. Verify `test`, `lint` and `build` locally before removing duplicated template files.
